...
def start_scheduler() -> AsyncIOScheduler:
    ...
    # Predict every 5 minutes at second 5 to ensure last 1m candle just closed.
    _scheduler.add_job(
        _job_predict_safely,
        CronTrigger(minute="*/5", second="5"),
        id="predict_round",
        max_instances=1,
        coalesce=True,
    ...
    # Settle every minute (cheap; only acts when target_at has passed).
    _scheduler.add_job(
        _job_settle_safely,
